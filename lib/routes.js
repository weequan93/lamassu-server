...
transactionRoutes.get("/cash_in_txs", function(req, res, next) {
  const { fromDate, deviceIds }  = req.body;
  
  const sql = `select * from cash_in_txs
    where device_id in ($1) 
    and created >= $2`

  const values = [deviceIds, fromDate]
...
transactionRoutes.get("/cash_out_txs", function (req, res, next) {
